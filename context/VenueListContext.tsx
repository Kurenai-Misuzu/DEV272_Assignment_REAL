...
// PROVIDER
export const VenueListContextProvider: React.FC<{
  ...
  return (
    <VenueListContext.Provider
      value={{ venueList, setVenueList, addVenue, updateVenue }}
    >
      {children}
    </VenueListContext.Provider>
  );
...
export const useVenueListContext = () => {
  ...
  if (!context) {
    throw new Error(
      "useVenueListContext must be used within it's appropriate provider!",
    );
  }
  return context;
};
